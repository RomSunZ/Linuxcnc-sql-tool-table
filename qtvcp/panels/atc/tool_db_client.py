...
class ToolDBClient:
	...
	def get_tool(self, toolno):
		"""Один инструмент — лёгкая операция ("get"), НЕ полный list_tools()
		(раньше get_tool() тянул весь список ради одной записи)."""
		resp = self.request("get", toolno=int(toolno))
		if not resp.get("ok"):
			return {}
		return resp.get("tool") or {}
